// Cached JSON-RPC Provider for BizenDAO

import { ethers } from 'ethers';
import { bizenCache } from './bizenDAOCache.js';
...
export class CachedJsonRpcProvider extends ethers.JsonRpcProvider {
  ...
  async send(method, params) {
    ...
    // Generate cache key
    const cacheKey = this.cache.generateKey(method, params, this.contractAddress);
  ...
  extractMethodFromCallData(data) {
    // Common BizenDAO contract method signatures
...
export function createCachedContract(address, abi, signerOrProvider, options = {}) {
