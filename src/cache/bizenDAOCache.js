// BizenDAO RPC Cache Implementation
...
export class BizenDAOCache {
  ...
  async init() {
    ...
    try {
      ...
      console.log('BizenDAO Cache initialized');
    } catch (error) {
  ...
  categorizeMethod(method) {
    const categories = {
      permanent: ['name', 'symbol', 'MAX_ROYALTY_BPS', 'burnHistory', 'tokenLocked'],
      longTerm: ['mintFee', 'royaltyInfo', 'creatorTokens', 'normalTokens', 'sbtTokens'],
      mediumTerm: ['tokenURI', 'creatorTokenCount', 'normalTokenCount', 'sbtTokenCount'],
