...
export class Header {
  ...
  async connectWallet() {
    ...
    try {
      ...
      // エラーを通知
      window.dispatchEvent(new CustomEvent('walletError', { detail: { error: error.message } }));
    }
  }
        ...
        <div class="wallet-container">
          ${this.isConnected ? `
            <button class="wallet-button connected" onclick="window.header.toggleDropdown()">
              ...
              <span class="wallet-address">${walletManager.formatAddress(this.account)}</span>
          ...
          ` : `
            <button class="wallet-button disconnected" onclick="window.header.connectWallet()" ${this.isLoading ? 'disabled' : ''}>
              ${this.isLoading ? `
