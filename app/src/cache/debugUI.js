...
export class CacheDebugUI {
  ...
  async updateMetrics() {
    ...
    const cacheSize = await bizenCache.db.getCacheSize();
    const cacheSizeMB = (cacheSize / 1024 / 1024).toFixed(2);

    this.container.innerHTML = `
      <h3 style="margin: 0 0 10px 0;">BizenDAO Cache Debug</h3>
  ...
  async clearGallery() {
    await bizenCache.db.clear('gallery_cache');
    alert('Gallery cache cleared!');
    this.updateMetrics();
  }

  async clearMetadata() {
    await bizenCache.db.clear('metadata_cache');
