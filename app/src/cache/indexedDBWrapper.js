// IndexedDB Wrapper for BizenDAO Cache
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('metadata_cache')) {
          const metadataStore = db.createObjectStore('metadata_cache', { keyPath: 'uri' });
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        request.onsuccess = async (event) => {
          const cursor = event.target.result;
          if (cursor) {
            currentSize += cursor.value.size || JSON.stringify(cursor.value).length;
